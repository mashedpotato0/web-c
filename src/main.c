#include <stdio.h>
#include <stdlib.h>
#include <string.h>
#include <SDL2/SDL.h>
#include "fetcher.h"
#include "processor.h"
#include "renderer.h"

void load_url(const char *url, dom_node **tree, int make_temp, int *scroll_y);

int check_click(dom_node *node, int mx, int my, char *url_buffer, dom_node **tree, int make_temp, int *scroll_y) {
    if (!node) return 0;

    if (node->tag && strcmp(node->tag, "a") == 0 && node->href) {
        int absolute_y = my + *scroll_y;
        if (mx >= node->layout.x && mx <= node->layout.x + node->layout.w &&
            absolute_y >= node->layout.y && absolute_y <= node->layout.y + node->layout.h) {

            printf("clicked link: %s\n", node->href);

        char target_url[512] = {0};
        if (strncmp(node->href, "http://", 7) == 0) {
            strcpy(target_url, node->href + 7);
        } else if (strncmp(node->href, "https://", 8) == 0) {
            strcpy(target_url, node->href + 8);
        } else if (strncmp(node->href, "//", 2) == 0) {
            strcpy(target_url, node->href + 2);
        } else if (node->href[0] == '/') {
            // relative link detected. extract the current hostname from the url buffer.
            char current_host[256] = {0};
            char *slash = strchr(url_buffer, '/');
            if (slash) {
                strncpy(current_host, url_buffer, slash - url_buffer);
            } else {
                strcpy(current_host, url_buffer);
            }
            snprintf(target_url, sizeof(target_url), "%s%s", current_host, node->href);
        } else {
            strcpy(target_url, node->href);
        }

        strncpy(url_buffer, target_url, 255);
        load_url(target_url, tree, make_temp, scroll_y);
        return 1;
            }
    }

    for (int i = 0; i < node->child_count; i++) {
        if (check_click(node->children[i], mx, my, url_buffer, tree, make_temp, scroll_y)) {
            return 1;
        }
    }

    return 0;
}
void load_url(const char *url, dom_node **tree, int make_temp, int *scroll_y) {
    char hostname[256] = {0};
    char path[256] = {0};
    const char *port = "80";

    if (scroll_y) *scroll_y = 0;

    const char *url_start = url;
    if (strncmp(url_start, "http://", 7) == 0) {
        url_start += 7;
    } else if (strncmp(url_start, "https://", 8) == 0) {
        url_start += 8;
        port = "443";
    } else if (strncmp(url_start, "//", 2) == 0) {
        url_start += 2;
    }

    char *slash = strchr(url_start, '/');
    if (slash) {
        strncpy(hostname, url_start, slash - url_start);
        strcpy(path, slash);
    } else {
        strcpy(hostname, url_start);
        strcpy(path, "/");
    }

    printf("fetching %s%s...\n", hostname, path);
    size_t data_size = 0;
    char *raw_data = fetch_html(hostname, port, path, &data_size);

    if (raw_data != NULL) {
        // only destroy the old tree if we actually got new data
        if (*tree) {
            free_tree(*tree);
            *tree = NULL;
        }
        *tree = process_response(raw_data, data_size, make_temp);
        free(raw_data);
    } else {
        printf("failed to fetch website data\n");
    }
}

int main() {
    int make_temp = 1;
    char url_buffer[256] = "www.gnu.org/";
    dom_node *tree = NULL;
    int scroll_y = 0;

    printf("starting browser process...\n");

    if (init_renderer() != 0) {
        return 1;
    }

    load_url(url_buffer, &tree, make_temp, &scroll_y);

    int running = 1;
    SDL_Event event;
    SDL_StartTextInput();

    while (running) {
        while (SDL_PollEvent(&event)) {
            if (event.type == SDL_QUIT) {
                running = 0;
            } else if (event.type == SDL_MOUSEBUTTONDOWN) {
                if (event.button.button == SDL_BUTTON_LEFT) {
                    if (event.button.y > 40) {
                        check_click(tree, event.button.x, event.button.y, url_buffer, &tree, make_temp, &scroll_y);
                    }
                }
            } else if (event.type == SDL_MOUSEWHEEL) {
                scroll_y -= event.wheel.y * 30;
                if (scroll_y < 0) scroll_y = 0;
            } else if (event.type == SDL_TEXTINPUT) {
                if (strlen(url_buffer) + strlen(event.text.text) < sizeof(url_buffer) - 1) {
                    strcat(url_buffer, event.text.text);
                }
            } else if (event.type == SDL_KEYDOWN) {
                if (event.key.keysym.sym == SDLK_BACKSPACE && strlen(url_buffer) > 0) {
                    url_buffer[strlen(url_buffer) - 1] = '\0';
                } else if (event.key.keysym.sym == SDLK_RETURN) {
                    load_url(url_buffer, &tree, make_temp, &scroll_y);
                } else if (event.key.keysym.sym == SDLK_DOWN) {
                    scroll_y += 30;
                } else if (event.key.keysym.sym == SDLK_UP) {
                    scroll_y -= 30;
                    if (scroll_y < 0) scroll_y = 0;
                }
            }
        }

        render_tree(tree, url_buffer, scroll_y);
        SDL_Delay(16);
    }

    printf("cleaning up and exiting...\n");
    if (tree) free_tree(tree);
    SDL_StopTextInput();
    cleanup_renderer();

    return 0;
}
